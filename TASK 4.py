...
def generate_key():
    ...
    return AESGCM.generate_key(bit_length=256)  # 32 bytes key
...
def encrypt(plaintext: str, key: bytes) -> str:
    ...
    """
    aesgcm = AESGCM(key)
    nonce = os.urandom(12)  # 96-bit nonce recommended for GCM [web:17]
    plaintext_bytes = plaintext.encode("utf-8")

    ciphertext = aesgcm.encrypt(nonce, plaintext_bytes, associated_data=None)

    # Store nonce + ciphertext together and base64-encode for easy transport
